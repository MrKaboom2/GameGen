"""Donate keys functionality for GameGen backend."""
...
def _load_donated_appids() -> set:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to read donated appids cache: {exc}")
        return set()
...
def _check_cache_staleness() -> None:
    ...
    if not os.path.exists(DONATED_APPIDS_FILE):
        ...
        try:
            ...
            logger.warn(f"GameGen: Failed to initialize donated appids cache: {exc}")
        return
        ...
        if first_line.startswith("DATE:"):
            try:
                ...
                if today - cached_date >= timedelta(days=7):
                    logger.log(
                        f"GameGen: Cache is {today - cached_date} old (since {cached_date}), "
                        "wiping."
                    )
                    with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                        f.write(f"DATE:{today.isoformat()}\n")
                ...
                logger.log("GameGen: Cache date format invalid, wiping.")
                with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                    f.write(f"DATE:{today.isoformat()}\n")
        else:
            # File exists but no date header, treat as stale
            logger.log("GameGen: Cache missing date header, wiping.")
            with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                f.write(f"DATE:{today.isoformat()}\n")
    except Exception as exc:
        logger.warn(f"GameGen: Failed to check cache staleness: {exc}")
...
def _save_donated_appids(appids: set) -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to save donated appids cache: {exc}")
...
def parse_config_vdf_decryption_keys(steam_path: str) -> List[Tuple[str, str]]:
    ...
    if not os.path.exists(config_path):
        logger.warn(f"GameGen: config.vdf not found at {config_path}")
        return []
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to read config.vdf: {exc}")
        return []

    try:
        vdf_data = _parse_vdf_simple(vdf_content)
    except Exception as exc:
        logger.warn(f"GameGen: Failed to parse config.vdf: {exc}")
        return []
...
def extract_valid_decryption_keys(steam_path: str) -> List[Tuple[str, str]]:
    ...
    if not steam_path or not os.path.exists(steam_path):
        logger.warn(f"GameGen: Invalid Steam path for donate keys: {steam_path}")
        return []

    logger.log("GameGen: Starting donate keys extraction...")
    ...
    for appid, key in all_pairs:
        if validate_appid_key_pair(appid, key):
            ...
            logger.log(
                f"GameGen: Invalid appid/key pair skipped: appid={appid!r}, "
                f"key_len={len(key)}, key_valid={bool(re.match(r'^[a-zA-Z0-9]+$', key))}"
            )

    logger.log(f"GameGen: Found {len(valid_pairs)} valid decryption key pairs")
    return valid_pairs
...
def send_donation_keys(pairs: List[Tuple[str, str]]) -> bool:
    ...
    """
    if not pairs:
        logger.log("GameGen: No keys to donate")
        return False
    ...
    if not new_pairs:
        logger.log(
            f"GameGen: All {len(pairs)} keys already donated, skipping request"
        )
        return True
    ...
    try:
        ...
        logger.log(
            f"GameGen: Sending {len(new_pairs)} new appid/key pairs "
        ...
        logger.log(f"GameGen: Donated AppIDs : {len(new_pairs)} - Resp : {status_code}")

        if status_code == 200:
            _save_donated_appids({appid for appid, _ in new_pairs})
            return True
        else:
            logger.log(f"GameGen: Donation request status : {status_code}")
            return False

    except Exception as exc:
        logger.warn(f"GameGen: Failed to send donation keys: {exc}")
        return False
