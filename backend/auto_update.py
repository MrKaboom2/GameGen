"""Auto-update utilities for the GameGen backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"GameGen updated to {new_version}. Please restart Steam."
        return "GameGen update applied. Please restart Steam."
...
def _fetch_github_latest(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            proxy_url = "https://GameGen.vercel.app/api/github-latest"
    ...
    if not zip_url and tag_name:
        zip_url = f"https://GameGen.vercel.app/api/get-plugin/{tag_name}"
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"GameGen updated to {latest_version}. Please restart Steam."
...
def _check_and_donate_keys() -> None:
    ...
    try:
        ...
        if not steam_path:
            logger.warn("GameGen: Cannot donate keys - Steam path not found")
            return
        ...
        if pairs:
            ...
            logger.log("GameGen: No valid keys found to donate")
            
    except Exception as exc:
        logger.warn(f"GameGen: Donate keys check failed: {exc}")
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"GameGen: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("GameGen: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"GameGen: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"GameGen: CheckForUpdatesNow failed: {exc}")
