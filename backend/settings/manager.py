...
def _detect_steam_language() -> Optional[str]:
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Valve\Steam") as key:
            ...
            if locale_code:
                logger.log(f"GameGen: detected Steam language '{steam_lang}' -> locale '{locale_code}'")
                _detected_steam_lang = locale_code
                return locale_code
            logger.log(f"GameGen: Steam language '{steam_lang}' has no matching locale, using default")
    except Exception as exc:
        logger.log(f"GameGen: could not read Steam language from registry: {exc}")
    return None
...
def _ensure_language_valid(values: Dict[str, Any]) -> bool:
    ...
    if current_language not in available_codes:
        logger.warn(
            f"GameGen: language '{current_language}' not available; "
...
def _available_theme_files() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if os.path.exists(themes_json_path):
            try:
                ...
                logger.warn(f"GameGen: Failed to parse themes.json: {exc}")
    except Exception:
        pass
    ...
    if not themes:
        try:
            ...
            logger.warn(f"GameGen: Failed to list theme files: {exc}")
    ...
    if not themes:
        ...
        logger.warn(f"GameGen: Using hardcoded theme list as fallback")
...
def _ensure_settings_dir() -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to ensure settings directory: {exc}")
...
def _load_settings_file() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to read settings file: {exc}")
        return {}
...
def _write_settings_file(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to persist settings file: {exc}")
...
def _validate_option_value(option: SettingOption, value: Any) -> Tuple[bool, Any, str | None]:
    ...
    if option.option_type == "select":
        ...
        if dynamic == "locales":
            ...
            try:
                logger.log(
                    "GameGen: validating locale option "
            ...
            try:
                logger.warn(
                    f"GameGen: invalid locale selection {value!r}; allowed codes "
            ...
            try:
                logger.log(
                    "GameGen: validating theme option "
                    f"value={candidate!r}, allowed={sorted(set(allowed_map.values()))}"
                )
            except Exception:
                pass
            ...
            try:
                logger.warn(
                    f"GameGen: invalid theme selection {value!r}; allowed themes "
                    f"{sorted(set(allowed_map.values()))}"
                )
            except Exception:
                pass
            ...
            try:
                logger.warn(
                    f"GameGen: invalid select option value {value!r}; allowed {sorted(allowed)}"
                )
            except Exception:
                pass
            return False, option.default, "Value not in list of allowed options"
    ...
    if first_launch:
        ...
        if detected:
            ...
            if detected in available_codes:
                ...
                logger.log(f"GameGen: first launch, auto-selected language '{detected}'")
            else:
                logger.log(
                    f"GameGen: detected locale '{detected}' not available "
...
def apply_settings_changes(changes: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with _SETTINGS_LOCK:
        ...
        for group_key, options_changes in changes.items():
            ...
            logger.log(f"GameGen: applying group {group_key} with payload {options_changes}")
            ...
            for option_key, value in options_changes.items():
                try:
                    logger.log(
                        f"GameGen: apply change request {group_key}.{option_key} -> {value!r}"
                    )
                except Exception:
                    pass
                ...
                try:
                    logger.log(
                        f"GameGen: validated {group_key}.{option_key}, "
        ...
        if not applied_changes and not language_changed:
            ...
            translations = get_locale_manager().get_locale_strings(language)
            logger.log(
                f"GameGen: no changes applied; returning cached values with language={language}"
            )
            return {
                "success": True,
                "values": values_snapshot,
        ...
        for option_key, previous, current_value in applied_changes:
            for callback in _CHANGE_HOOKS.get(option_key, []):
                try:
                    ...
                    logger.warn(f"GameGen: settings hook failed for {option_key}: {exc}")

        translations = get_locale_manager().get_locale_strings(language)

        logger.log(
            f"GameGen: apply_settings_changes final language={language}, values={values_snapshot}"
        )
        return {
            "success": True,
            "values": values_snapshot,
