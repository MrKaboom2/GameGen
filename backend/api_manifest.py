"""Management of the GameGen API manifest (free API list)."""
...
def fetch_free_apis_now(content_script_query: str = "") -> str:
    ...
    client = ensure_http_client("GameGen: FetchFreeApisNow")
    try:
        logger.log("GameGen: FetchFreeApisNow invoked")
        manifest_text = ""

        try:
            logger.log(f"GameGen: Fetching manifest from {API_MANIFEST_URL}")
            resp = client.get(API_MANIFEST_URL, follow_redirects=True)
            logger.log(f"GameGen: Manifest response: status={resp.status_code}")
            resp.raise_for_status()
            manifest_text = resp.text
            logger.log("GameGen: Fetched manifest from primary URL")
        except Exception as primary_err:
            logger.warn(f"GameGen: Primary manifest URL failed ({primary_err}), trying proxy...")
            try:
                logger.log(f"GameGen: Fetching manifest from proxy {API_MANIFEST_PROXY_URL}")
                resp = client.get(
                    API_MANIFEST_PROXY_URL,
                    follow_redirects=True,
                    timeout=HTTP_PROXY_TIMEOUT_SECONDS,
                )
                logger.log(f"GameGen: Proxy manifest response: status={resp.status_code}")
                resp.raise_for_status()
                manifest_text = resp.text
                logger.log("GameGen: Fetched manifest from proxy URL")
            except Exception as proxy_err:
                logger.warn(f"GameGen: Proxy manifest URL also failed: {proxy_err}")
                return json.dumps(
        ...
        logger.warn(f"GameGen: FetchFreeApisNow failed: {exc}")
...
def load_api_manifest() -> List[Dict[str, Any]]:
    ...
    if normalized and normalized != text:
        try:
            ...
            logger.log("GameGen: Normalized api.json")
        except Exception:
            pass
        text = normalized
    ...
    try:
        ...
        logger.error(f"GameGen: Failed to parse api.json: {exc}")
        return []
...
def get_api_list(content_script_query: str = "") -> str:
    ...
    try:
        ...
        logger.error(f"GameGen: Failed to get API list: {exc}")
