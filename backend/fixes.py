...
FIXES_INDEX_URL = "https://index.GameGen.work/fixes-index.json"
...
def _fetch_fixes_index() -> Optional[Dict]:
    ...
    try:
        client = ensure_http_client("GameGen: FixesIndex")
        ...
        if resp.status_code == 200:
            ...
            logger.log(f"GameGen: Fixes index loaded ({len(generic_set)} generic, {len(online_set)} online)")
            return index
        else:
            logger.warn(f"GameGen: Fixes index fetch returned {resp.status_code}")
    except Exception as exc:
        logger.warn(f"GameGen: Failed to fetch fixes index: {exc}")
...
def check_for_fixes(appid: int) -> str:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to fetch game name for {appid}: {exc}")
    ...
    if index is not None:
        generic_url = f"https://files.GameGen.work/GameBypasses/{appid}.zip"
        online_url = f"https://files.GameGen.work/OnlineFix1/{appid}.zip"
        ...
        logger.log(f"GameGen: Fix check for {appid} via index: generic={appid in index['generic']}, online={appid in index['online']}")
    else:
        # Fallback: HEAD requests if index is unavailable
        logger.warn(f"GameGen: Fixes index unavailable, falling back to HEAD requests for {appid}")
        client = ensure_http_client("GameGen: CheckForFixes")
        try:
            generic_url = f"https://files.GameGen.work/GameBypasses/{appid}.zip"
            ...
            logger.warn(f"GameGen: Generic fix check failed for {appid}: {exc}")
        ...
        try:
            ...
            logger.warn(f"GameGen: Online-fix check failed for {appid}: {exc}")

    return json.dumps(result)


def _download_and_extract_fix(appid: int, download_url: str, install_path: str, fix_type: str, game_name: str = ""):
    client = ensure_http_client("GameGen: fix download")
    try:
        ...
        logger.log(f"GameGen: Downloading {fix_type} from {download_url}")

        with client.stream("GET", download_url, follow_redirects=True, timeout=30) as resp:
            logger.log(f"GameGen: Fix download response for {appid}: status={resp.status_code}")
            ...
            with open(dest_zip, "wb") as output:
                for chunk in resp.iter_bytes():
                    ...
                    if state.get("status") == "cancelled":
                        logger.log(f"GameGen: Fix download cancelled before writing chunk for {appid}")
                    ...
                    if _get_fix_download_state(appid).get("status") == "cancelled":
                        logger.log(f"GameGen: Fix download cancelled for {appid}")
                        raise RuntimeError("cancelled")

        logger.log(f"GameGen: Download complete, extracting to {install_path}")
        ...
        with zipfile.ZipFile(dest_zip, "r") as archive:
            ...
            if _get_fix_download_state(appid).get("status") == "cancelled":
                logger.log(f"GameGen: Fix extraction cancelled before start for {appid}")
                raise RuntimeError("cancelled")

            if len(top_level_entries) == 1 and appid_folder.rstrip("/") in top_level_entries:
                logger.log(f"GameGen: Found single folder {appid} in zip, extracting its contents")
                for member in archive.namelist():
                    if member.startswith(appid_folder) and member != appid_folder:
                        ...
                        if not _is_safe_path(install_path, target_path):
                            logger.warn(f"GameGen: Skipping potentially unsafe path in zip: {member}")
                            continue
                        ...
                        source.close()
                        if _get_fix_download_state(appid).get("status") == "cancelled":
                            logger.log(f"GameGen: Fix extraction cancelled mid-process for {appid}")
                            raise RuntimeError("cancelled")
            else:
                logger.log(f"GameGen: Extracting all zip contents to {install_path}")
                for member in archive.namelist():
                    if member.endswith("/"):
                        continue
                    # Validate path doesn't escape install directory (prevent path traversal)
        ...
        if _get_fix_download_state(appid).get("status") == "cancelled":
            logger.log(f"GameGen: Fix cancelled after extraction for {appid}")
            raise RuntimeError("cancelled")
                ...
                if ini_relative_path:
                    ...
                    if os.path.exists(ini_full_path):
                        ...
                        if updated_contents != contents:
                            ...
                            logger.log(f"GameGen: Updated unsteam.ini with appid {appid}")
                        else:
                            logger.log("GameGen: unsteam.ini did not contain <appid> placeholder or was already updated")
                    else:
                        logger.warn(f"GameGen: Expected unsteam.ini at {ini_full_path} but file not found")
                else:
                    logger.warn("GameGen: Extracted files do not include unsteam.ini for Online Fix (Unsteam)")
            except Exception as exc:
                logger.warn(f"GameGen: Failed to update unsteam.ini: {exc}")

        log_file_path = os.path.join(install_path, f"GameGen-fix-log-{appid}.log")
        try:
            ...
            logger.log(f"GameGen: Appended fix log at {log_file_path} with {len(extracted_files)} files")
        except Exception as exc:
            logger.warn(f"GameGen: Failed to create fix log file: {exc}")

        logger.log(f"GameGen: {fix_type} applied successfully to {install_path}")
        ...
        if str(exc) == "cancelled":
            ...
            return
        logger.warn(f"GameGen: Failed to apply fix: {exc}")
...
def apply_game_fix(appid: int, download_url: str, install_path: str, fix_type: str = "", game_name: str = "") -> str:
    ...
    logger.log(f"GameGen: ApplyGameFix appid={appid}, fixType={fix_type}")
...
def cancel_apply_fix(appid: int) -> str:
    ...
    _set_fix_download_state(appid, {"status": "cancelled", "success": False, "error": "Cancelled by user"})
    logger.log(f"GameGen: CancelApplyFix requested for appid={appid}")
    return json.dumps({"success": True})


def _unfix_game_worker(appid: int, install_path: str, fix_date: str = None):
    try:
        logger.log(f"GameGen: Starting un-fix for appid {appid}, fix_date={fix_date}")
        log_file_path = os.path.join(install_path, f"GameGen-fix-log-{appid}.log")
        ...
        try:
            ...
            logger.log(f"GameGen: Found {len(files_to_delete)} unique files to remove from log")
        except Exception as exc:
            logger.warn(f"GameGen: Failed to read log file: {exc}")
        ...
        for file_path in files_to_delete:
            try:
                ...
                if os.path.exists(full_path):
                    os.remove(full_path)
                    deleted_count += 1
                    logger.log(f"GameGen: Deleted {file_path}")
            except Exception as exc:
                logger.warn(f"GameGen: Failed to delete {file_path}: {exc}")

        logger.log(f"GameGen: Deleted {deleted_count}/{len(files_to_delete)} files")
        ...
        if remaining_fixes:
            ...
            try:
                ...
                logger.log(f"GameGen: Updated log file, {len(remaining_fixes)} fixes remaining")
            except Exception as exc:
                logger.warn(f"GameGen: Failed to update log file: {exc}")
        else:
            # No fixes remaining, delete the log file
            try:
                os.remove(log_file_path)
                logger.log(f"GameGen: Deleted log file {log_file_path}")
            except Exception as exc:
                logger.warn(f"GameGen: Failed to delete log file: {exc}")

        _set_unfix_state(appid, {"status": "done", "success": True, "filesRemoved": deleted_count})

    except Exception as exc:
        logger.warn(f"GameGen: Un-fix failed: {exc}")
...
def unfix_game(appid: int, install_path: str = "", fix_date: str = "") -> str:
    ...
    logger.log(f"GameGen: UnFixGame appid={appid}, path={resolved_path}, fix_date={fix_date}")
...
def get_installed_fixes() -> str:
    """Scan all Steam library folders for games with GameGen fix logs."""
    try:
        ...
        try:
            ...
            logger.warn(f"GameGen: Failed to parse libraryfolders.vdf: {exc}")
            ...
            try:
                for filename in os.listdir(steamapps_path):
                    ...
                    try:
                        ...
                        # Check for GameGen fix log
                        log_file_path = os.path.join(full_install_path, f"GameGen-fix-log-{appid}.log")
                        if os.path.exists(log_file_path):
                            ...
                            try:
                                ...
                                logger.warn(f"GameGen: Failed to parse fix log for {appid}: {exc}")

                    except Exception as exc:
                        logger.warn(f"GameGen: Failed to process manifest {filename}: {exc}")
                        continue

            except Exception as exc:
                logger.warn(f"GameGen: Failed to scan library {lib_path}: {exc}")
                continue

        return json.dumps({"success": True, "fixes": installed_fixes})

    except Exception as exc:
        logger.warn(f"GameGen: Failed to get installed fixes: {exc}")
