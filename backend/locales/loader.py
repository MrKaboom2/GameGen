...
def _ensure_locales_dir() -> None:
    try:
        ...
        logger.warn(f"GameGen: Failed to ensure locales dir: {exc}")
...
def _read_locale_file(locale: str) -> Tuple[Dict[str, Any], Dict[str, str]]:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to read locale file {path}: {exc}")
        return {}, {}
...
def _write_locale_file(locale: str, meta: Dict[str, Any], strings: Dict[str, str]) -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to write locale file {locale}: {exc}")
...
class LocaleManager:
    ...
    def refresh(self) -> None:
        with self._lock:
            ...
            if not strings:
                logger.warn("GameGen: Default locale en.json is empty or missing.")
                strings = {}
