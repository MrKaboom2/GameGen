"""Steam-related utilities used across GameGen backend modules."""
...
def detect_steam_install_path() -> str:
    ...
    logger.log(f"GameGen: Steam install path set to {_STEAM_INSTALL_PATH}")
...
def _find_steam_path() -> str:
    ...
    if sys.platform.startswith("win") and winreg:
        try:
            ...
            logger.warn(f"GameGen: Failed to read Steam path from registry: {exc}")
...
def has_lua_for_app(appid: int) -> bool:
    try:
        ...
        logger.error(f"GameGen (steam_utils): Error checking Lua scripts for app {appid}: {exc}")
        return False
...
def get_game_install_path_response(appid: int) -> Dict[str, any]:
    ...
    if not os.path.exists(library_vdf_path):
        logger.warn(f"GameGen: libraryfolders.vdf not found at {library_vdf_path}")
    ...
    if not library_path:
        logger.log(
            f"GameGen: appid {appid} not in libraryfolders.vdf, searching all libraries for appmanifest"
        )
        for lib_path in all_library_paths:
            ...
            if os.path.exists(candidate_path):
                ...
                logger.log(f"GameGen: Found appmanifest at {appmanifest_path}")
                break
    else:
    ...
    if not library_path or not appmanifest_path or not os.path.exists(appmanifest_path):
        logger.log(f"GameGen: appmanifest not found for {appid} in any library")
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to parse appmanifest: {exc}")
        return {"success": False, "error": "Failed to parse appmanifest"}

    app_state = manifest_data.get("AppState", {})
    install_dir = app_state.get("installdir", "")
    if not install_dir:
        logger.warn(f"GameGen: installdir not found in appmanifest for {appid}")
    ...
    if not os.path.exists(full_install_path):
        logger.warn(f"GameGen: Game install path does not exist: {full_install_path}")
        return {"success": False, "error": "Game directory not found"}

    logger.log(f"GameGen: Game install path for {appid}: {full_install_path}")
    return {
        "success": True,
...
def open_game_folder(path: str) -> bool:
    ...
    try:
        ...
        logger.warn(f"GameGen: Failed to open game folder: {exc}")
        return False
